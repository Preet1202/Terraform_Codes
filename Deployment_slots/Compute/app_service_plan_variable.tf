...
variable "backend_service_plan_name" {
  type        = string
  description = "The name which should be used for backend(rewardportalapi) service plan."
  default     = "asp-backend"
}

variable "backend_service_plan_os_type" {
  type        = string
  description = "The O/S type for the App Services to be hosted in this plan. Possible values include `Windows`, `Linux` and `WindowsContainer`."
  default     = "Windows" #"Windows", "Linux", "WindowsContainer"
}
